...
def download_model_from_drive(output_dir: str = "output"):
    ...
    # peft saves adapter weights as .safetensors by default in current
    # versions, but older versions (and the original spec) used .bin --
    # try both since we don't control which one was uploaded.
    found = download_file("lora_adapters_adapter_model.safetensors",
                          f"{output_dir}/lora_adapters/adapter_model.safetensors")
    if not found:
